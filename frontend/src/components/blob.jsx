...
const VoiceReactivePlasma = ({ sensitivity = 2.5, smoothing = 0.85, idle = true }) => {
  ...
  const initScene = useCallback((container) => {
    ...
    const plasmaMat = new THREE.ShaderMaterial({
      ...
      vertexShader: `
        ...
        void main(){
          ...
          float displacement = 1.0 + uAudioLevel * 0.35;
          float noiseDisp = snoise(position * 3.0 + uAudioLevel * 2.0) * uAudioLevel * 0.15;
    ...
    const pMat = new THREE.ShaderMaterial({
      ...
      vertexShader: `
        ...
        void main(){
          vec3 pos = position;
          float audioScale = 1.0 + uAudioLevel * 0.4;
          pos *= audioScale;
          pos.y += sin(uTime * 0.2 + pos.x) * 0.02 * (1.0 + uAudioLevel * 3.0);
          pos.x += cos(uTime * 0.15 + pos.z) * 0.02 * (1.0 + uAudioLevel * 3.0);
          vec4 mv = modelViewMatrix * vec4(pos, 1.0);
          gl_Position = projectionMatrix * mv;
          float baseSize = (8.0 * aSize + 4.0) * (1.0 + uAudioLevel * 2.0);
          gl_PointSize = baseSize * (1.0 / -mv.z);
          vAlpha = (0.8 + 0.2 * sin(uTime + aSize * 10.0)) * (0.5 + uAudioLevel * 1.5);
        }
      `,
      fragmentShader: `
        uniform vec3 uColor;
    ...
    const initAudio = async () => {
      try {
        ...
        console.warn('Microphone access denied. Running in idle animation mode.', err);
      }
    };
    ...
    const getAudioLevel = () => {
      ...
      return Math.min(avg * sensitivity, 1.0);
    };
    ...
    const animate = () => {
      ...
      if (idle && rawVolume < 0.01) {
        rawVolume = 0.05 + Math.sin(t * 1.2) * 0.03 + Math.sin(t * 0.7) * 0.02;
      }

      smoothedVolume += (rawVolume - smoothedVolume) * 0.12;
      const audioLevel = smoothedVolume;

      // Update plasma
      const timeScale = 0.78 + audioLevel * 2.0;
      ...
      // Scale shell with audio
      const shellScale = 1.0 + audioLevel * 0.35;
      ...
      // Group rotation — speeds up with audio
      const rotMult = 1.0 + audioLevel * 3.0;
